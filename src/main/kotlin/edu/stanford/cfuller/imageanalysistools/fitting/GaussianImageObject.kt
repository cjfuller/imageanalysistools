...
class GaussianImageObject : ImageObject {
    ...
    override fun fitPosition(p: ParameterDictionary) {
        ...
        for (channelIndex in 0..numChannels - 1) {
            ...
            for (i in this.xValues!!.indices) {
                residualSumSquared += Math.pow(GaussianFitter3D.fitResidual(
                        functionValues!![i], xValues!![i], yValues!![i], zValues!![i], fitParameters), 2.0)
            ...
            val variance = this.xValues!!.indices.sumByDouble { Math.pow(functionValues!![it] - mean, 2.0) }
    ...
    private inner class DI1Func : UnivariateFunction {
    ...
    private inner class ErrIntFunc : UnivariateFunction {
        private var b: Double = 0.toDouble()
        private var n: Double = 0.toDouble()
        private var A: Double = 0.toDouble()
        private var sa_z: Double = 0.toDouble()
        ...
        private val lgi: LegendreGaussIntegrator = LegendreGaussIntegrator(5, 10, 1000)
        private val di1: DI1Func = DI1Func()
