...
class ParameterDictionary : Serializable {
    private var parameters: Hashtable<String, MutableList<Parameter>>? = null
    private constructor() {}
    ...
    fun discardIllegalArguments(legalArguments: Array<String>) {
        ...
        for (key in this.parameters!!.keys) {
            val legal = legalArguments.indices.any { legalArguments[it] == key }
            if (!legal) {
                toRemove.add(key)
            }
        }
        toRemove.forEach { this.parameters!!.remove(it) }
    }
    ...
    fun getTypeForKey(key: String, i: Int): ParameterType {
        return this.parameters!![key]!![i].type
    }

    private fun parameterForStringValue(key: String, value: String): Parameter {
        return Parameter(key, key, ParameterType.STRING_T, value, "")
    }
    ...
    companion object {
        ...
        fun emptyDictionary(): ParameterDictionary {
            ...
            parameters.parameters = Hashtable<String, MutableList<Parameter>>()
            return parameters
        }
