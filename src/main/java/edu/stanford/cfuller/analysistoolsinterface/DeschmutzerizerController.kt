...
class DeschmutzerizerController : TaskController() {
    internal var dw: DeschmutzerizerWindow? = null
    internal var dieh: DeschmutzerizerInputEventHandler? = null
    internal var originalImageWindow: ImageWindow? = null
    internal var maskWindow: ImageWindow? = null
    internal var currentOriginalImage: Image? = null
    internal var currentMaskImage: Image? = null
    internal var currentLabeledMaskImage: Image? = null
    internal var selectedRegions: HashSet<Int>? = null
    internal var colorCodedMaskDisplay: ImagePlus? = null
    internal var originalImageDisplay: ImagePlus? = null
    internal var filenamesToProcess: MutableList<String>? = null
    ...
    fun processBox(start: Point, end: Point) {
        ...
        for (ic in regionNumberCheck) {
            ...
            if (tempSelectedRegions.contains(regionNumberCheck.getValue(ic).toInt()) &&
                    !this.selectedRegions!!.contains(this.currentLabeledMaskImage!!.getValue(ic).toInt())) {
                ...
                if (y < y0) y0 = y
                if (y >= yf) yf = y + 1
            } else if (tempSelectedRegions.contains(regionNumberCheck.getValue(ic).toInt()) &&
                    this.selectedRegions!!.contains(this.currentLabeledMaskImage!!.getValue(ic).toInt())) {
                this.colorCodedMaskDisplay!!.setPositionWithoutUpdate(c + 1, z + 1, t + 1)
                val ip = this.colorCodedMaskDisplay!!.processor
    ...
    private fun finish() {
        ...
        if (this.lastParametersProcessed != null) {
            try {
                DataSummary.SummarizeData(
                        File(this.getOutputFilename(this.lastDataFilenameProcessed!!)).parent,
                        File(this.lastParametersProcessed!!).parent)
    ...
    private fun finishCurrentImage() {
        ...
        try {
            ...
            while (currentLine != null) {
                val split = currentLine.split(" ".toRegex()).dropLastWhile(String::isEmpty).toTypedArray()
                var regionNumber = 0
                try {
            ...
            q.allMeasurements
                    .asSequence()
                    .filterNot { this.selectedRegions!!.contains(it.featureID.toInt()) }
                    .forEach { outputQ.addMeasurement(it) }
    ...
    private fun readCurrentMaskAndDataFilenames(nextImageToProcess: String) {
        ...
        val parametersFilename: String? = parametersDirFile.listFiles()!!
                .firstOrNull { it.name.matches((".*" + nextToProcessFile.name + ".*").toRegex()) }
                ?.absolutePath
        ...
        val outputImages = am.outputImages
    ...
    private fun processNextImage() {
        ...
        display
                .asSequence()
                .filter { display.getValue(it) > 0 }
                .forEach { display.setValue(it, java.lang.Float.MAX_VALUE) }

        val origMask = ImageFactory.createWritable(this.currentMaskImage!!).toImagePlus()
        this.colorCodedMaskDisplay = origMask.createHyperStack(
                "mask", this.currentMaskImage!!.dimensionSizes[ImageCoordinate.C],
                this.currentMaskImage!!.dimensionSizes[ImageCoordinate.Z],
                this.currentMaskImage!!.dimensionSizes[ImageCoordinate.T], 24) //24 = RGB
        ...
        if (this.maskWindow == null || this.originalImageWindow == null) {
            ...
            this.originalImageWindow!!.setLocationAndSize(
                    this.originalImageWindow!!.location.getX().toInt(),
                    (this.originalImageWindow!!.location.getY() + this.dw!!.height).toInt(),
                    500,
                    (500.0 * this.currentMaskImage!!.dimensionSizes[ImageCoordinate.Y] / this.currentMaskImage!!.dimensionSizes[ImageCoordinate.X]).toInt())
            this.maskWindow!!.setLocationRelativeTo(this.dw)
            this.maskWindow!!.setLocationAndSize(
                    (this.maskWindow!!.location.getX() + this.dw!!.width * 1.1).toInt(),
                    (this.maskWindow!!.location.getY() + this.dw!!.height / 2).toInt(),
                    500,
                    (500.0 * this.currentMaskImage!!.dimensionSizes[ImageCoordinate.Y] / this.currentMaskImage!!.dimensionSizes[ImageCoordinate.X]).toInt())
